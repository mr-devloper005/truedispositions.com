...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  const taskLabel = task === 'pdf' ? 'PDF resource' : SITE_CONFIG.tasks.find((item) => item.key === task)?.label || 'Resource'
  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-[2rem] border border-[var(--editable-border)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-2xl ${strong ? 'md:col-span-2' : ''}`}>
      ...
      <div className="p-5 sm:p-6">
        {!image ? <span className="rounded-full bg-[var(--editable-page-text,#112E81)] px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-white">{taskLabel}</span> : null}
        <h2 className="mt-4 line-clamp-3 text-2xl font-black leading-[0.95] tracking-[-0.06em] text-[var(--editable-page-text,#112E81)]">{post.title}</h2>
        {summary ? <p className="mt-4 line-clamp-3 text-sm font-semibold leading-7 text-[var(--editable-page-text,#112E81)]/65">{summary}</p> : null}
        <span className="mt-5 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] opacity-60 group-hover:opacity-100">Open resource <ArrowRight className="h-4 w-4" /></span>
      </div>
    </Link>
  )
}
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--editable-page-bg,#AACCD6)] text-[var(--editable-page-text,#112E81)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[2.5rem] border border-[var(--editable-border)] bg-white/70 p-6 shadow-[0_30px_90px_rgba(17,46,129,0.10)] backdrop-blur md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
            ...
            <form action="/search" className="self-end rounded-[2rem] border border-[var(--editable-border)] bg-[var(--editable-page-bg,#AACCD6)] p-4 sm:p-5">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-2xl bg-[var(--editable-page-text,#112E81)] px-6 text-sm font-black uppercase tracking-[0.18em] text-white transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            ...
            <Link href="/pdf" className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-white px-5 py-3 text-sm font-black">Browse PDF library <ArrowRight className="h-4 w-4" /></Link>
          </div>
          ...
          {results.length ? (
            ...
            <div className="mt-8 rounded-[2rem] border border-dashed border-[var(--editable-border)] bg-white/70 p-10 text-center">
              <p className="text-2xl font-black tracking-[-0.04em]">No matching documents found.</p>
              <p className="mt-3 text-sm font-semibold opacity-60">Try a different keyword, document type, or category.</p>
            </div>
          )}
        </section>
      </main>
